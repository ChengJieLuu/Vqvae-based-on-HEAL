...
from opencood.utils.camera_utils import img_to_tensor, img_transform, normalize_img, sample_augmentation
from opencood.utils.common_utils import compute_iou, convert_format, merge_features_to_dict, read_json
from opencood.utils.heter_utils import Adaptor
from opencood.utils.pcd_utils import downsample_lidar_minimum, mask_ego_points, shuffle_points
from opencood.utils.pose_utils import add_noise_data_dict
from opencood.utils.transformation_utils import get_pairwise_transformation, x1_to_x2
...
class DiffPreProcessor:
    ...
    def preprocess(self, pcd: np.ndarray):
        ...
        for points in pcd:
            ...
            img_row, img_col = self._get_resolution(self.cav_lidar_range, self.ratio)
        ...
        if len(pcd) == 1:
            bev_maps = bev_maps[0]
        # , "dep_maps": dep_maps, "origin_lidar": pcd, "pcd_inputs_ids": self._get_inputs_ids("")
        return bev_maps
...
def getDiffFusionDataset(cls):
    """
    cls: the Basedataset.
    """

    class DiffFusionDataset(cls):
        def __init__(self, params, visualize, train=True):
            ...
            for modality_name, modal_setting in params["heter"]["modality_setting"].items():
                ...
                if modal_setting["sensor_type"] == "lidar":
                    # TODO: 原来配置文件里面的 `preprocess` 是为了存储具体参数用的, `core_method` 根本没有用到啊
            ...
            # TODO: 传递参数的时候需要注意一下
            self.diff_pre_processor = DiffPreProcessor(params["preprocess"])
        ...
        def get_item_single_car(self, selected_cav_base, ego_cav_base):
            ...
            # 别合在一起写, 不然调试的时候不方便 pcd
            bev_maps = self.diff_pre_processor.preprocess(lidar_np)
            selected_cav_processed.update({"bev_maps": bev_maps})
        ...
        def __getitem__(self, idx):
            ...
            for _i, cav_id in enumerate(cav_id_list):
                ...
                bev_maps_list.append(selected_cav_processed["bev_maps"])
            ...
            processed_data_dict["ego"].update({
                ...
                # TODO: 这里是否需要 stack 呢?
                "bev_maps_list": torch.tensor(bev_maps_list[0]) if len(bev_maps_list) == 1 else torch.stack([torch.tensor(bev_maps_list[i]) for i in range(len(bev_maps_list))]),
            })
        ...
        def collate_batch_train(self, batch):
            ...
            for i in range(len(batch)):
                ...
                bev_maps_list_list.append(ego_dict["bev_maps_list"])
            ...
            output_dict["ego"].update({
                ...
                "pcd": torch.stack(bev_maps_list_list),
            })
