...
class VQVAE(nn.Module):
    def __init__(self, in_channels, embedding_dim=256, hidden_dim=512 ,num_embeddings=1024, num_res_blocks=4):
        super().__init__()
        self.embedding_dim = embedding_dim
        self.variance = torch.load('variance.pth')['data_variance']
        # self.variance = 0.541417
    ...
    def forward(self, x):
        ...
        # 重建损失（除以方差进行归一化）
        recon_loss = F.mse_loss(x_recon, x) / 0.225
    ...
    #     # 重建损失（除以方差进行归一化）
    #     recon_loss = F.mse_loss(x_recon, x) / self.variance
    ...
    #     return x_recon, total_loss, vq_loss